...
def parameter_all_Description(table: str, contents: str, parameters: str) -> str:
    ...
    Gas_parameter={
        'CenterOfMass': '''Spatial position of the center of mass, 
                         which in general differs from the geometrical center of the Voronoi cell (the offset should be small). 
                         Comoving coordinate.''',
        ...
        'ElectronAbundance': '''Fractional electron number density with respect to the total hydrogen number density, 
                              so ne=ElectronAbundance*nH where nH=XH*rho/mp. Use with caution for star-forming gas (see comment below for NeutralHydrogenAbundance).''',
                              
        'EnergyDissipation': '''Shock finder output: the dissipated energy rate (amount of kinetic energy irreversibly transformed into thermal energy). 
                             Note units correspond to (Energy/Time). ''',
                             
        'GFM_AGNRadiation': '''Bolometric intensity (physical units) at the position of this cell arising from the radiation fields of nearby AGN. 
                             One should divide by 4π to obtain the flux at this location, in the sense of F=L/(4πR2).''',
                             
        'GFM_CoolingRate': '''The instantaneous net cooling rate experienced by this gas cell, in cgs units (e.g. Λnet/n2H).''',
        
        'GFM_Metallicity': '''The ratio MZ/Mtotal where MZ is the total mass all metal elements (above He). 
                            Is NOT in solar units. To convert to solar metallicity, divide by 0.0127 (the primordial solar metallicity).''',
                            
        'GFM_Metals': '''Individual abundances of nine species: H, He, C, N, O, Ne, Mg, Si, Fe (in this order). 
                       Each is the dimensionless ratio of mass in that species to the total gas cell mass. 
                       The tenth entry contains the 'total' of all other (i.e. untracked) metals.''',
                       
        'GFM_MetalsTagged': '''Six additional metal-origin tracking fields in this order: SNIa (0), SNII (1), AGB (2), NSNS (3), FeSNIa (4), FeSNII (5). 
                             Each keeps track of heavy elements arising from particular processes. Full description below.''',
        ...
        'InternalEnergy': '''Internal (thermal) energy per unit mass for this gas cell. See FAQ for conversion to gas temperature. 
                           Use with caution for star-forming gas, as this corresponds to the 'effective' temperature of the equation of state, 
                           which is not a physical temperature. Note: this field has "corrected" values, and is generally recommended for all uses, 
                           see the data release background for details.''',
                           
        'InternalEnergyOld': '''Old internal (thermal) energy per unit mass for this gas cell. See FAQ for conversion to gas temperature. 
                              This field holds the original values, and is not recommended for use, see the data release background for details. (!) Note that subboxes do not have corrected values, 
                              so the InternalEnergy field for subboxes contains the uncorrected values, and no InternalEnergyOld exists.''',
        ...
        'NeutralHydrogenAbundance': '''Fraction of the hydrogen cell mass (or density) in neutral hydrogen, 
                                    so nH0=NeutralHydrogenAbundance*nH. (So note that nH+=nH-nH0). 
                                    Use with caution for star-forming gas, as the calculation is based on the 'effective' temperature of the equation of state, 
                                    which is not a physical temperature.''',
                                    
        'ParticleIDs': '''The unique ID (uint64) of this gas cell. Constant for the duration of the simulation. 
                            May cease to exist (as gas) in a future snapshot due to conversion into a star/wind particle, accretion into a BH, 
                            or a derefinement event.''',
                            
        'Potential': '''Gravitational potential energy.''',
        
        'StarFormationRate':  '''Instantaneous star formation rate of this gas cell.''',
        
        'SubfindDMDensity': '''The local total comoving mass density, 
                            estimated using the standard cubic-spline SPH kernel over all DM particles within a radius of SubfindHsml.''',
                            
        'SubfindDensity': '''The local total comoving mass density, 
                            estimated using the standard cubic-spline SPH kernel over all particles/cells within a radius of SubfindHsml.''',
    ...
    Star_parameter={
        ...
        'GFM_MetalsTagged': '''See entry under gas. This field is identical for star particles, 
                          and note that it is simply inherited at the time of formation from the gas cell from which the star was born. 
                           It does not then evolve or change in any way (i.e. no self-enrichment), 
                           so these values describe the 'inherited' wind/SN/NSNS material from the gas.''',
                           
        'GFM_StellarFormationTime': '''The exact time (given as the scalefactor) when this star was formed. 
                                    Note: The only differentiation between a real star (>0) and a wind phase gas cell (<=0) is the sign of this quantity.''',
                                    
        'GFM_StellarPhotometrics': '''Stellar magnitudes in eight bands: U, B, V, K, g, r, i, z. 
                                    In detail, these are: Buser's X filter, where X=U,B3,V (Vega magnitudes), 
        ...
        'StellarHsml': '''The comoving radius of the sphere centered on this particle enclosing the 32±1 nearest particles of this same type. 
    ...
    BH_parameter={
        'BH_BPressure': '''The mean magnetic pressure of gas cells within a radius of BH_Hsml, 
                        kernel and volume weighted (kernel weight clipped at a maximum of wt=2.5). Units are those of MagneticField2. 
                        Note: is still in Heavyside-Lorentz, not Gauss, so multiply by 4π to be unit consistent with MagneticField.''',
                        
        'BH_CumEgyInjection_QM': '''Cumulative amount of thermal AGN feedback energy injected into surrounding gas in the high accretion-state (quasar) mode, 
                                    total over the entire lifetime of this blackhole. Field summed during BH-BH merger.''',
                                    
        'BH_CumEgyInjection_RM': '''Cumulative amount of kinetic AGN feedback energy injected into surrounding gas in the low accretion-state (wind) mode, 
                                    total over the entire lifetime of this blackhole. Field summed during BH-BH merger.''',
                                    
        'BH_CumMassGrowth_QM': '''Cumulative mass accreted onto the BH in the high accretion-state (quasar) mode, 
                                    total over the entire lifetime of this blackhole. Field summed during BH-BH merger.''',
                                    
        'BH_CumMassGrowth_RM': '''Cumulative mass accreted onto the BH in the low accretion-state (kinetic wind) mode, 
                                    total over the entire lifetime of this blackhole. Field summed during BH-BH merger.''',
        ...
        'BH_Pressure': '''Physical gas pressure (in comoving units) near the BH, defined as (gama-1)*rho*u, 
        where rho is the local comoving gas density (BH_Density, as above) u is BH_U (defined below)''',
        
        'BH_Progs': '''Total number of BHs that have merged into this BH.''',
        
        'BH_U': '''Thermal energy per unit mass in quasar-heated bubbles near the BH. Used to define the BH_Pressure. 
                    Not to be confused with the "radio mode" bubbles injected via the unified feedback model.''',
    ...
    FoF_halos={
        ...
        'GroupCM': '''Center of mass of the group, computed as the sum of the mass weighted relative coordinates of all particles/cells in the group, of all types. 
                        Comoving coordinate. (Available only for the Illustris-3 run)''',
                        
        'GroupFirstSub': '''Index into the Subhalo table of the first (i.e. central/primary) Subfind subhalo within this FoF group. 
                            The subhalos of a group are ordered in descending total number of bound particles/cells. 
                            The first/central subhalo is usually, but not always, the most massive. 
                            Note: This value is signed (or should be interpreted as signed)! 
                            In this case, a value of -1 indicates that this FoF group has no subhalos.''',
                            
        'GroupGasMetalFractions': '''Individual abundances: H, He, C, N, O, Ne, Mg, Si, Fe, total (in this order). Each is the dimensionless ratio of the total mass in that species divided by the total gas mass, for all gas cells in the group. 
                                        The tenth entry contains the 'total' of all other (i.e. untracked) metals.''',
        ...
        'GroupLenType': '''Integer counter of the total number of particles/cells, split into the six different types, in this group. 
                        Note: Wind phase cells are counted as stars (type 4) for GroupLenType.''',
                        
        'GroupMass': '''Sum of the individual masses of every particle/cell, of all types, in this group.''',
        
        'GroupMassType': '''Sum of the individual masses of every particle/cell, split into the six different types, in this group. 
                            Note: Wind phase cells are counted as gas (type 0) for GroupMassType.''',
        ...
        'GroupStarMetalFractions': '''Individual abundances: H, He, C, N, O, Ne, Mg, Si, Fe, total (in this order). Each is the dimensionless ratio of the total mass in that species divided by the total stellar mass, for all stars in the group. 
                                        The tenth entry contains the 'total' of all other (i.e. untracked) metals.''',
        ...
        'Group_M_TopHat200': '''Total Mass of this group enclosed in a sphere whose mean density is Δc times the critical density' of the Universe, at the time the halo is considered. 
                                Δc derives from the solution of the collapse of a spherical top-hat perturbation (fitting formula from Bryan+ 1998). 
                                The subscript 200 can be ignored.''',
    ...
    Subfind_subhalos={
        'SubhaloFlag': '''Flag field indicating suitability of this subhalo for certain types of analysis. 
                            If zero, this subhalo should generally be excluded, and is not thought to be of cosmological origin. 
                            That is, it may have formed within an existing halo, or is possibly a baryonic fragment of a disk or other galactic structure identified by Subfind. 
                            If one, this subhalo should be considered a 'galaxy' or 'satellite' of cosmological origin. (Note: always true for centrals). 
                            This field is only present for baryonic runs, and is absent for dark matter only runs. See the data release background for details.''',
        ...
        'SubhaloBfldDisk': '''The square root of the volume weighted value of B2
                                for all gas cells within the canonical two times the stellar half mass radius. 
                                This value gives a magnetic field strength which would have the same amount of mean magnetic energy as the galaxy cells. 
                                (*) Only available for full snapshots.''',
        
        'SubhaloBfldHalo': '''The square root of the volume weighted value of B2 for all gas cells in the subhalo. 
                                This value gives a magnetic field strength which would have the same amount of mean magnetic energy as the subhalo cells. 
                                (*) Only available for full snapshots.''',
        ...
        'SubhaloGasMetalFractions': '''Individual abundances: H, He, C, N, O, Ne, Mg, Si, Fe, total (in this order). 
                                        Each is the dimensionless ratio of the total mass in that species divided by the total gas mass, 
                                        both restricted to gas cells within twice the stellar half mass radius. 
                                        The tenth entry contains the 'total' of all other (i.e. untracked) metals.''',
        ...
        'SubhaloGasMetallicity': '''Mass-weighted average metallicity (Mz/Mtot, where Z = any element above He) of the gas cells bound to this Subhalo, 
        ...
        'SubhaloMassType': '''Total mass of all member particle/cells which are bound to this Subhalo, separated by type. Particle/cells bound to subhaloes of this Subhalo are NOT accounted for. 
                            Note: Wind phase cells are counted as gas (type 0) for SubhaloMassType.''',
                            
        'SubhaloParent': '''Index back into this same Subhalo table of the unique Subfind host/parent of this Subhalo. 
                            This index is local to the group (i.e. 2 indicates the third most massive subhalo of the parent halo of this subhalo, not the third most massive of the whole snapshot). 
                            The values are often zero for all subhalos of a group, indicating that there is no resolved hierarchical structure in that group, 
                            beyond the primary subhalo having as direct children all of the secondary subhalos.''',
        ...
        'SubhaloStarMetalFractions': '''Individual abundances: H, He, C, N, O, Ne, Mg, Si, Fe, total (in this order). 
                                        Each is the dimensionless ratio of the total mass in that species divided by the total stellar mass, 
                                        both restricted to stars within twice the stellar half mass radius. 
                                        The tenth entry contains the 'total' of all other (i.e. untracked) metals.''',
        ...
        'SubhaloStarMetallicity': '''Mass-weighted average metallicity (Mz/Mtot, where Z = any element above He) of the star particles bound to this Subhalo, 
                                        but restricted to stars within twice the stellar half mass radius.''',
